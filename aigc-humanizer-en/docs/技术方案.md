...
| 改写 | `humanizer/` | RuleBased / Api / ApiMock | `HUMANIZER_ADAPTER` |
...
flowchart TB
    ...
    subgraph Humanizer["改写适配器"]
        ...
        H2[ApiHumanizer<br>ai-text-humanizer.com]
        H3[ApiMockHumanizer<br>随机 sleep 1-1.5s]
    end
...
> **Mock 适配器**：`sapling_mock / rule_based_mock / api_mock` 用于本地测试"改写/检测进行中"的等待界面，不真实请求外部 API，每次随机 sleep 1-1.5s，行为/耗时与真实 API 对齐。

**模式三：异步后台任务**
- 改写（无论余额充足直接改写，还是支付后改写）统一提交到 `rewrite_executor`（`ThreadPoolExecutor(max_workers=5)`）执行 `do_background_rewrite`
- 不阻塞支付 Webhook 响应 / API 响应，保证及时返回
- 启动时 `recover_processing_orders()` 扫描并恢复卡在 `processing` 状态的订单（处理服务器重启场景）
...
| 改写引擎可替换 | 适配器模式 — RuleBasedHumanizer + ApiHumanizer |
...
sequenceDiagram
    ...
    alt 余额充足
        ...
        App->>Executor: submit(do_background_rewrite)
...
- **支付后改写**：`process_payment_success` 充值 + 扣费后提交同一后台线程。支付弹窗轮询 `/api/payment-status` 到 `processing` 后，切到主页面进度页，再轮询 `/api/rewrite-progress`。
...
- **ApiHumanizer** — 调用 `ai-text-humanizer.com` API，支持分块（>2000 词）、重试（3 次）、频控
- **ApiMockHumanizer** — 随机 sleep 1-1.5s 返回假文本，测试等待界面
...
**频控**：改写请求数超过 `RATE_LIMIT_MAX_REQUESTS=30` 时，每次改写请求后 sleep `RATE_LIMIT_SLEEP=1.0s`，防止超 60 次/分钟上限。
...
| **适配器配置** | `PAYMENT_ADAPTER=mock/alipay`，`HUMANIZER_ADAPTER=rule_based/api/api_mock`，`AI_DETECTOR_ADAPTER=rule_based/sapling/originality/*_mock` |
| **改写粒度默认** | `REWRITE_MODE_DEFAULT=median`；`REWRITE_MEDIAN_PARAS=3`，`REWRITE_HIGH_PARAS=5` |
| **改写字数上限** | `REWRITE_MAX_WORDS=2000`；`RATE_LIMIT_MAX_REQUESTS=30`，`RATE_LIMIT_SLEEP=1.0` |
