...
logger = logging.getLogger("app.humanizer.api_mock")


class ApiMockHumanizer(ApiHumanizer):
    """
    模拟 API 改写：不真实请求 ai-text-humanizer.com，每次改写随机 sleep 1-1.5s。
    用于测试"改写进行中"等待界面，行为/耗时与真实 API 对齐。

    仅覆盖 _call_api（mock 改写动作），其余分块/频控/结构保护逻辑完全复用 ApiHumanizer。
    """

    backend_label = "api_mock"
    ...
    def _call_api(self, text):
        ...
        start = time.time()
        logger.info(
            "rewrite stage=rewrite backend=api_mock action=call_start words=%d sleep=%.2fs",
        ...
        result = '++++++++\n' + ' '.join(lines)
        logger.info(
            "rewrite stage=rewrite backend=api_mock action=call_ok words=%d out_chars=%d elapsed=%.0fms",
